<template>
    ...
    <div class="p-4 flex-grow flex flex-col gap-3">
        <div class="-mx-4 -mt-4 py-8 text-white flex flex-col items-center shadow-sm" :style="`background-color: ${vm.color}`">
            <div class="text-3xl font-semibold">{{ vm.subjectAverage }}</div>
        ...
        <div class="-mx-4 p-4 grid grid-cols-3 grid-rows-1">
            <div class="w-full text-center">
                <div class="textarea-xl font-semibold">{{ vm.notes.length }}</div>
                <div class="font-thin text-sm">Total Grades</div>
            </div>
            <div class="w-full text-center">
                <div class="textarea-xl font-semibold text-success">{{ vm.bestNote }}</div>
                <div class="font-thin text-sm">Best Grade</div>
            </div>
            <div class="w-full text-center">
                <div class="textarea-xl font-semibold text-error">{{ vm.worstNote }}</div>
        ...
        <div>
            ...
            <div class="w-full border border-base-200 p-1">
                <LineGraph :values="vm.lineGraphValues" :labels="vm.lineGraphLabels" :color="vm.color" />
            </div>
        </div>
        <div>
            <h2 class="font-semibold pb-1">Grades</h2>
            <div v-for="note of vm.notes" class="card card-sm bg-base-100 border border-base-300">
                <div class="card-body">
                    <div class="flex gap-2">
                        <div class="avatar">
                            <div
                                ...
                                <SubjectIconRender :icon="SubjectIcon.BOOK" class="w-6 h-6"/>
                            </div>
                        </div>
                        <div class="flex-1">
                            <div class="font-semibold text-lg mb-1">{{ note }}</div>
                            <div class="text-xs">{{ note.date.format("DD MMM. YYYY") }}</div>
                        </div>
                        <div class="text-lg font-semibold h-full">{{ note.value }}</div>
                    </div>
                </div>
            </div>
        </div>
...
import { SubjectIcon } from "@/models/subject.model";
